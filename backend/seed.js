...
const sampleInternships = [
  {
    ...
    applicationDeadline: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000), // 30 days ahead
  },
  {
...
const seedData = async () => {
  try {
    ...
    console.log('Cleared existing User, Internship, and Application collections.');
    ...
    console.log('Admin user created: admin@ims.com / admin123');
    ...
    console.log('Student user created: student@ims.com / student123 (Skills: Python, SQL, React, JavaScript, HTML, CSS)');

    // 3. Create Sample Internships
    const createdInternships = await Internship.insertMany(sampleInternships);
    console.log(`Created ${createdInternships.length} sample internships.`);

    // 4. Create an initial sample application for demo
    await Application.create({
      student: studentUser._id,
      internship: createdInternships[0]._id, // TechCorp Full Stack
      status: 'Under Review',
    ...
    await Application.create({
      student: studentUser._id,
      internship: createdInternships[2]._id, // PixelCraft Frontend
      status: 'Shortlisted',
      appliedAt: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000),
    });

    console.log('Created 2 sample applications for demonstration.');
